...
export function makeApiGatewayV2Event(values: any = {}) {
  ...
  if (!mergedEvent.requestContext.http.path)
    mergedEvent.requestContext.http.path = values.path;
  if (!mergedEvent.requestContext.http.method)
    mergedEvent.requestContext.http.method = values.httpMethod;

  mergedEvent.headers = convertMultiValueHeadersToHeaders({
...
export function makeApiGatewayV2Response(
  ...
  values.headers = convertMultiValueHeadersToHeaders({
    multiValueHeaders: values.multiValueHeaders,
  });
