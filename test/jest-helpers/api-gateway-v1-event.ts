...
export function makeApiGatewayV1Event(values: any = {}) {
  const baseEvent = clone(apiGatewayV1Event);
  const mergedEvent = mergeDeep(baseEvent, values);

  if (!mergedEvent.pathParameters.proxy)
    mergedEvent.pathParameters.proxy = values.path.replace(/^\//, '');
  if (!mergedEvent.requestContext.path)
    mergedEvent.requestContext.path = `${mergedEvent.requestContext.stage}${mergedEvent.path}`;

  return mergedEvent;
}

export function makeApiGatewayV1Response(values = {}) {
  const baseResponse = {
    body: '',
    isBase64Encoded: false,
    statusCode: 200,
