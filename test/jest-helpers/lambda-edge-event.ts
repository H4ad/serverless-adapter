...
export function makeLambdaEdgeEvent(values: any = {}) {
  ...
  if (
    !mergedEvent.request.querystring &&
    values.multiValueQueryStringParameters
  ) {
    const multiValueQueryStringParametersToArray: any = [];

    Object.entries(values.multiValueQueryStringParameters).forEach(
      ([qKey, qValues]: [any, any[]]) => {
  ...
  if (!mergedEvent.request.method)
    mergedEvent.request.method = values.httpMethod;

  if (!mergedEvent.request.body.data) {
    mergedEvent.request.body.data = values.isBase64Encoded
      ? values.body
      : Buffer.from(values.body || '', 'binary').toString('base64');
  }
...
export function convertMultiValueHeaders({ multiValueHeaders }) {
  const headers = {};

  if (!multiValueHeaders) return headers;

  Object.entries(multiValueHeaders).forEach(([key, value]: [any, any[]]) => {
...
export function makeLambdaEdgeResponse(values: any = {}) {
  ...
  values.headers = convertMultiValueHeaders({
    multiValueHeaders: values.multiValueHeaders,
  });
