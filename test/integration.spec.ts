  ...
  (eventSourceName, frameworkName) => {
    ...
    test('GET HTML', async () => {
      ...
      router.get('/', (req, res) => {
        ...
        const eventPath =
          event.path || event.rawPath || event.Records[0].cf.request.uri;
        res.render('index', {
          path: eventPath,
        });
      });
    ...
    test('resolutionMode = CALLBACK', done => {
      ...
      const serverlessExpressInstanceWithCallbackResolutionMode =
    ...
    test('resolutionMode = CONTEXT', done => {
      ...
      const serverlessExpressInstanceWithContextResolutionMode =
        serverlessExpress({ app, resolutionMode: 'CONTEXT' });

      void serverlessExpressInstanceWithContextResolutionMode(event, context);
    });
    ...
    test('base64 encoded request', async () => {
      ...
      const expectedResponse = makeResponse({
        ...
        multiValueHeaders: {
          'content-length': ['27'],
          etag: ['W/"1b-bCkbUU5T9Cepc4SpN5w/iwctZZw"'],
        },
        statusCode: 200,
      });
      expect(response).toEqual(expectedResponse);
    ...
    test('set-cookie', async () => {
      ...
      const expectedSetCookieHeaders = ['Foo=bar; Path=/', 'Fizz=buzz; Path=/'];
      const expectedResponse = makeResponse({
        eventSourceName,
        body: '{}',
