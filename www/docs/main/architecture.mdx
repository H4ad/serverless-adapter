...
So I refactored [@vendia/serverless-express](https://github.com/vendia/serverless-express) with five extension layers:
Framework, Network, Handler, Resolver and Adapter.
...
The [NetworkContract](https://github.com/H4ad/serverless-adapter/tree/main/src/contracts/network.contract.ts) is responsible for creating the
[IncomingMessage](https://nodejs.org/api/http.html#class-httpincomingmessage) and
[ServerResponse](https://nodejs.org/api/http.html#class-httpserverresponse) objects sent to your framework, then extracting the collected body and headers after the framework finishes.
...
manage to call each layer of abstraction to return a response. Built-in adapter-driven handlers resolve an adapter, ask the network layer to create the framework request/response objects, forward them to the framework, and pass the extracted response data back to the adapter.
...
Well, with these five contracts, you'll be able to add support to any cloud that exists (no more excuses not to use
cloud X with NodeJS).
