...
Check the [Azure Handler](../../handlers/azure) docs to make sure you have all the necessary settings to work with this adapter.
...
This adapter was created to work with runtime 4.x, [see the docs](https://docs.microsoft.com/en-us/azure/azure-functions/functions-versions?tabs=azure-cli%2Cin-process%2Cv4&pivots=programming-language-javascript#changing-version-of-apps-in-azure)
...
This option only changes responses that were already marked as binary by [setBinarySettings](../../getting-started/customizing#setbinarysettings). The adapter only decodes base64 response bodies into `Uint8Array`; it does not convert plain text, JSON, or any other non-base64 body format.
...
You MUST use the [PromiseResolver](../../resolvers/promise) combined with [Azure Handler](../../handlers/azure) to make this adapter work.
