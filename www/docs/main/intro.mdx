...
By design we have these contracts that define the layers of the library: Frameworks, Network, Adapters, Resolvers and Handlers.
...
The default network implementation, `DEFAULT_NETWORK`, creates `ServerlessRequest` and `ServerlessResponse` objects for you. You only need to customize it with `setNetwork` when you want to tune `DefaultNetwork` or replace those Node.js request/response shims.

Currently, we support these clouds (handlers):

- Azure by using ([AzureHandlerV3](./handlers/azure))
