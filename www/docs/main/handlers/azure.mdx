...
We only support v2/v3 of the @azure/functions, if you want support for v4, leave a comment at [this issue](https://github.com/H4ad/serverless-adapter/issues/371)
...
you can set the `useContextLogWhenInternalLogger` option to `false` for [AzureHandlerOptions](../../api/Handlers/AzureHandlerV3/AzureHandlerV3Options).

```typescript
import { AzureHandlerV3 } from '@h4ad/serverless-adapter/handlers/azure';

const handler = new AzureHandlerV3({
...
You `MUST` use [PromiseResolver](../resolvers/promise) to integrate with `AzureHandlerV3`, other resolvers didn't work with Azure.
