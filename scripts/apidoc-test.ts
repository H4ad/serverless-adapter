...
function parseTSDoc(foundComment: IFoundComment): void {
  ...
  const stringBuiler = new StringBuilder();
  const options: IMarkdownEmitterOptions = {};

  console.log(emitter.emit(stringBuiler, docComment, options));
...
export function advancedDemo(): void {
  ...
  const inputFilename: string = path.resolve(
    path.join(__dirname, '..', 'dist', 'serverless-adapter.d.ts'),
  );
  ...
  // Compile the input
  console.log(
    'Invoking the TypeScript compiler to analyze assets/advanced-input.ts...',
  );
